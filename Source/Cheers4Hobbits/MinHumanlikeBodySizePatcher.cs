...
namespace Cheers4Hobbits
{
	...
	{
		static readonly MethodInfo pawnBodySizeGetter = typeof(Pawn).GetProperty(nameof(Pawn.BodySize)).GetGetMethod();
		static readonly MethodInfo minHumanLikeBodySizeMethod =
		...
		public static IEnumerable<CodeInstruction> MinHumanlikeBodySizeTranspiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			{
				if (instruction.opcode == OpCodes.Callvirt && instruction.operand == pawnBodySizeGetter)
					yield return new CodeInstruction(OpCodes.Call, minHumanLikeBodySizeMethod);
				else
